...
pygame.display.set_caption("Magic 8 Ball") #title of game

#defining colors with RGB format:
WHITE = (255,255,255)
BLACK = (0,0,0)
DARK = (20,20,20)
RED = (255,0,0)
...
font = pygame.font.SysFont('Arial', 53, bold=False, italic=False)#Our font (Arial, size 53)
...
def magicball(): #main function which will randomly generate a response
	num = random.randint(1,20)
	if num==1:
		response = pygame.image.load('magic8ballimages/as_i_see_it.png')#1/20 chance each image will be portrayed
		window.blit(response, (40,20))
		pygame.display.update()
	elif num==2:
		response = pygame.image.load('magic8ballimages/ask_again_later.png')#each image is a different 8 ball response
		window.blit(response, (40,20))
		pygame.display.update()
	elif num==3:
...
while run:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT: #if user exits, end the code
			run = False
		if keys[pygame.K_1] or keys[pygame.K_KP1] and start: #if user presses one, prepare the screen to ask another question
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_SPACE: 
					print("ASKING 8 BALL: ")
					dots = [".","..", "...", "....", ".....", "......", ".......", "........", ".........", ".........."]
					for d in range(len(dots)): #for loop producing the loading dots while loading response
					    pygame.time.delay(200)
					    sys.stdout.write("\r" + dots[d % len(dots)])
					    sys.stdout.flush()
					print("\n")
					magicball() #run the main function
					game_on = False
